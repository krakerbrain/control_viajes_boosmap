...
$sqlUsuario = $con->prepare("SELECT idusuario, correo FROM usuarios WHERE nombre = :nombreUsuario");
    ...
    <div class="row-cols-lg-2">
        <form action="" method="post" class="form-group mt-3 mx-auto">
            <div class="input-group">
                ...
                <input type="text" name="usuario" id="usuario" class="form-control" value="<?= $_SESSION['usuario'] ?>"
                    disabled>
            </div>
        ...
        .done(function(resp) {
            ...
            try {
                ...
                if (correo_actualizado == 'true' && clave_actualizada == 'false') {
                    ...
                    respuesta =
                        'El password ha sido actualizado correctamente. <br> Se recomienda <a href="<?= $_ENV['URL_SESSION'] ?>">CERRAR SESIÓN</a> y acceder con la nueva clave'
                } else if (correo_actualizado == 'true' && clave_actualizada == 'true') {
                    respuesta =
                        `${data.respuesta} <br> Se recomienda <a href="<?= $_ENV['URL_SESSION'] ?>">CERRAR SESIÓN</a> y acceder con la nueva clave`
                } else {
